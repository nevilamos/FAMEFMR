...
makeGS_Summary <- function(myFHAnalysis = FHAnalysis,
  ...
  GS_Summary_Long <- dplyr::left_join(GS_Summary_Long, GS_LUT)#join Growth Stage names to output table

  # change names
  data.table::setnames(GS_Summary, old = TimeNames, new = GS_Names <- paste0("GS_", TimeNames))
