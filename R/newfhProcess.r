#' Make processed FH for FAME stage1
#'
...
fhProcess1<-function(
  ...
  if(n_WithIntersects>3){
    ...
    cores=min(c(cores[1]-2,myPreppedFH$inWithIntersects))
...
#'
#' @return ouput processed fire history with unique polygons each with sequence of fires and firetypes
#' @export
#'
#' @examples
...
#'   there is no overlap)
#' @param end.SEASON last season required, if NULL then maximum value in fire history scenario used
#' @param max_interval maximum inter fire interval after a HIGH fire for which subsequent fire is reassigned to HIGH, if 0 then no reassignment)
#'
#' @return completed Fire Analysis Simple feature collection of polygons and/or multipolygons
#'  with columns for Fire Season sequences, inter-fire intervals Last fire type,last fire 
#'  date and last burned year for each season selected in the input range.   
#' @export
#'
#' @examples
...
prepFH<-function(inFH = myFH){
  # calculate the number of grid cells to be used in parallelizing function.
  #at the moment based on area the FH covers over 10^8 square meters
  #give the the square root of the number of cells in the grid
  ...
  hasIntersects<-unlist(lapply(hasData,FUN = function(x){max(unlist(lapply(as.list(sf::st_intersects(x)),length)))>0}))
  return(list("hasData"=hasData,"squareside" =squareside,"hasIntersects"=hasIntersects))
}
...
#' @param inFHLayer Layer name if inFH has more than one layer 
#' ( for instance in a .gpkg) this allows selection of a particular layer,
#' otherwise frist layer is used (Default = NULL)
#' @return
#' @export
#'
#' @examples
