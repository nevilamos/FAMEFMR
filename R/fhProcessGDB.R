#' Main Fire History Fire Sequence analysis function GDB version needs testing more
#'
...
#' @param readGDB Boolean TRUE if the input is a featureclass in a geodatabase, not a shapefile. Default = False
#' @param gdbPath path to geodatabase containing rawFH if readGDB =  TRUE, otherwise NULL.
#' @param rawFH path to the input fire history shapefile usually  provided in settings. Alternatively the name of a feature class in gdb if readGDB =  TRUE
#'
...
fhProcessGDB<-function(
  ...
  # read in shapefile
  if (readGDB){
    myDF <- sf::st_read(dsn = gdbPath,layer =rawFH )
  }else{
  ...
  #geometry was removed in the GDB version because it was throwing an error
