############################################################################
# Function calcBBTFI_2.r is used to calculate the ...                       #####-----brief description of overall function
# written by nevil.amos@delwp.vic.gov.au
############################################################################


## FUNCTION unlistPivot_wider ----------------------------------------------
                        ...
                        makeBBTFIrasters = makeBBTFIrasters )            #the dataframe read from a csv that gives the lookup table from EVD to MinTFI_LO, MinTFI_HI and MaxTFI)
{
  ...
  # x<-left_join(FH_ID,OutTab)                                              #####-----remove
  # x<-as.matrix(x)                                                         #####-----remove
  # mode(x)<-"integer"                                                      #####-----remove
  ...
  # make bushfire and planned burn type matrices                            #####-----if this is right, can you link to firetype look up table
  TYPE_HI <- TYPE == 2                                               # TRUE where thetype of the first fire is HI(2)
  TYPE_LO <- TYPE == 1                                               # TRUE where thetype of the first fire is LO(1)
  
  # refine matrices by FH_ID
  ...
  # this needs a little more annotation about what/why e.g. make 3 dataframes (WIDEish, LONG, WIDE) and their differences   #####-----are the three outputs findamentally different? does each user use each one, or only one of them?
  ...
  if(makeBBTFIrasters){
    ...
    # writeRaster(r,file.path(ResultsDir,"BBTFI_Rasters","FirstBBTFI.tif"),overwrite=T,datatype="INT2S",options="COMPRESS=LZW")           #####-----delete all
    # r<-raster(nrows=nrow(r), ncols=ncol(r),ext=extent(r),crs=crs(r),vals=NULL)
